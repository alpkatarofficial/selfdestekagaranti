...
export default async function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  let product: RapooProduct | null = null
  let error: string | null = null

  try {
    let { data, error: fetchError } = await supabase.from("rapoo").select("*").eq("id", productId).limit(1)
    ...
    if ((!data || data.length === 0) && isNumeric) {
      try {
        ...
        const numericRes = await supabase.from("rapoo").select("*").eq("id", numericId).limit(1)
    ...
    if (!data || data.length === 0) {
      const alt = await supabase.from("rapoo").select("*").eq("model_name", productId).limit(1)
      if (alt.error) throw alt.error
      data = alt.data
    }

    if (data && data.length > 0) {
      product = data[0] as RapooProduct
    } else {
