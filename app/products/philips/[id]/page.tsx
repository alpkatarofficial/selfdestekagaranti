...
export default async function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  let product: PhilipsProduct | null = null
  let error: string | null = null

  try {
    let { data, error: fetchError } = await supabase.from("philips").select("*").eq("id", productId).limit(1)
    ...
    if ((!data || data.length === 0) && isNumeric) {
      try {
        ...
        const numericRes = await supabase.from("philips").select("*").eq("id", numericId).limit(1)
    ...
    if (!data || data.length === 0) {
      const alt = await supabase.from("philips").select("*").eq("model_name", productId).limit(1)
