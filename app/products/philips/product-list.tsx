...
import type { PhilipsProduct } from "./[id]/index"

export default function PhilipsProductList({ products = [] }: { products: PhilipsProduct[] }) {
  ...
  if (!products || products.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">Philips ürünleri yakında eklenecek.</p>
      </div>
    )
  }
      ...
      {filteredProducts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map((product) => (
            <Link key={product.id} href={`/products/philips/${product.id}`}>
              <Card className="group cursor-pointer overflow-hidden rounded-lg shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                <div className="relative h-48 w-full overflow-hidden bg-gray-100">
                  <Image
                    src={product.image_url || "/placeholder.svg?height=192&width=300&query=Philips"}
                    alt={product.name}
                    fill
