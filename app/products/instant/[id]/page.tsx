...
export default async function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  let product: InstantProduct | null = null
  let error: string | null = null

  try {
    // Try by `id` first (string)
    let { data, error: fetchError } = await supabase.from("instant").select("*").eq("id", productId).limit(1)
    ...
    if ((!data || data.length === 0) && isNumeric) {
      try {
        ...
        const numericRes = await supabase.from("instant").select("*").eq("id", numericId).limit(1)
        if (numericRes.error) throw numericRes.error
        if (numericRes.data && numericRes.data.length > 0) {
          data = numericRes.data
        }
      } catch (innerErr) {
        // ignore numeric fallback errors here; we'll try model_name next
    ...
    if (!data || data.length === 0) {
      const alt = await supabase.from("instant").select("*").eq("model_name", productId).limit(1)
      if (alt.error) throw alt.error
      data = alt.data
    }

    if (data && data.length > 0) {
      product = data[0] as InstantProduct
    } else {
