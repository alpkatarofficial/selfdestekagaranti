...
interface Product {
  id: string | number  // Instant uses integer IDs, but we handle both
  name: string
  model_name?: string
  category: string
  description: string
...
interface InstantClientPageProps {
  products: Product[]
}

// Category display names mapping
const categoryDisplayNames: Record<string, string> = {
  "flash-bellek": "Flash Bellek",
  "hafiza-karti": "Hafıza Kartı",
  "ssd": "SSD",
  "tasinabilir-ssd": "Taşınabilir SSD",
  "hub": "Hub",
  "kablo": "Kablo",
}

export default function InstantClientPage({ products }: InstantClientPageProps) {
      ...
      {filteredProducts.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {filteredProducts.map((product) => (
            <Link key={product.id} href={`/products/instant/${product.id}`} className="group">
